...
def handle_add_waveform_dir_command(args, logger):
    ...
    try:
        ...
        if psybase.project.defaultWaveclient is None:
            logger.error("No default waveclient found. Don't know for which waveclient the waveform dir should be added.")

        default_wc = psybase.project.defaultWaveclient
        waveclient = psybase.project.waveclient[default_wc]
        wf_dir = wf_dir_orm(waveclient.name,
                            args.path,
                            args.description,
                            args.file_ext,
                            '',
