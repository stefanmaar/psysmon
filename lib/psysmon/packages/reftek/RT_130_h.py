...
class DT (object) :
    ...
    def decode (self, buf) :
        ...
        if ret == None :
            raise CorruptPacketError ("Found corrupt packet header in DT.decode")
        ...
        if   dt.data_format == 0x16 :
            ...
            sys.stderr.write ("Warning: Can't parse overscaled data formats\n")
            dt = None
            ...
            sys.stderr.write ("Warning: Unrecognized data format: %s\n" % hex (dt.data_format))
            dt = None
...
class EH_object (object) :
    __slots__ = ('EventNumber', 'DataStream', 'Flags', 'DataFormat', 'TriggerTime', 
...
def event_header () :
    EH = construct.Struct ("EH",
                           construct.BitStruct ("BIN",
                                                ...
                                                construct.BitField ("Flags", 8),
                                                construct.BitField ("DataFormat", 8)),
                           construct.String ("TriggerTime", 33),
...
class EH (object) :
    __keys__ = ("BIN.PacketHeader", "BIN.EventNumber", "BIN.DataStream", 
                "BIN.Reserved", "BIN.Flags", "BIN.DataFormat", "TriggerTime", 
